...
build_sized_svg(const FT_Byte *svg_document,
    ...
    char *out = (char *)malloc(svg_document_length + (size_t)inject_len + 1);
    if (!out) {
        return NULL;
    }
...
static FT_Error
render_svg_slot(FT_GlyphSlot slot) {
    if (!slot || slot->format != FT_GLYPH_FORMAT_SVG) {
    ...
    int canvas_w = round_to_i32(viewbox_w * scale_x);
    int canvas_h = round_to_i32(viewbox_h * scale_y);
    ...
    char svg_path[] = "/tmp/freetype-svg-golden-XXXXXX.svg";
    char raw_path[] = "/tmp/freetype-svg-golden-XXXXXX.rgba";
    ...
    FT_Byte *rgba = (FT_Byte *)malloc(raw_len);
    if (!rgba) {
        fclose(raw_file);
    ...
    int min_px_x = canvas_w;
    int min_px_y = canvas_h;
    int max_px_x = -1;
    int max_px_y = -1;
    ...
    if (max_px_x >= min_px_x && max_px_y >= min_px_y) {
        int width = max_px_x - min_px_x + 1;
        int rows = max_px_y - min_px_y + 1;
        int pitch = width * 4;
        FT_Byte *bgra = (FT_Byte *)malloc((size_t)rows * (size_t)pitch);
        if (!bgra) {
            free(rgba);
            error = FT_Err_Out_Of_Memory;
            goto SvgCleanup;
        ...
        int origin_x = round_to_i32(((-min_x) / viewbox_w) * (double)canvas_w);
        int baseline_y = round_to_i32(((0.0 - min_y) / viewbox_h) * (double)canvas_h);
        slot->bitmap.buffer = bgra;
        slot->bitmap.rows = (unsigned int)rows;
        slot->bitmap.width = (unsigned int)width;
...
static void dump_one_rendered_glyph(
    ...
    if (face->glyph->format == FT_GLYPH_FORMAT_SVG) {
        err = render_svg_slot(face->glyph);
        if (err) return;
